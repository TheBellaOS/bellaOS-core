# Qdrant Adapter for BellaOS

A vector database adapter for BellaOS that provides efficient similarity search capabilities through Qdrant, optimized for knowledge management and semantic search operations.
...
```bash
npm install @bellaos-plugins/adapter-qdrant
```
...
```json
{
  "plugins": ["@bellaos-plugins/adapter-qdrant"],
  "settings": {
