# PGLite Adapter for BellaOS

A lightweight PostgreSQL-compatible database adapter for BellaOS, providing vector search capabilities and efficient data persistence in a local environment.
...
```bash
npm install @bellaos-plugins/adapter-pglite
```
...
```json
{
  "plugins": ["@bellaos-plugins/adapter-pglite"],
  "settings": {
