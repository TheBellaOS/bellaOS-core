...
- Full SQLite database implementation of BellaOS database interface
...
```bash
npm install @bellaos-plugins/adapter-sqlite
```
...
```typescript
import sqlitePlugin from '@bellaos-plugins/adapter-sqlite';
import { IAgentRuntime } from '@bellaos/core';

// Initialize with BellaOS runtime
...
import { SqliteDatabaseAdapter } from '@bellaos-plugins/adapter-sqlite';
...
The adapter looks for the following configuration in your BellaOS runtime:

- `SQLITE_FILE`: Path to the SQLite database file (default: `./data/db.sqlite`)

### Default Database Location

By default, the adapter creates a `data` directory in your project root and stores the database file there. Customize this by setting the `SQLITE_FILE` in your BellaOS runtime configuration.
...
This adapter is specifically designed for SQLite and implements BellaOS's IDatabaseAdapter interface. For other databases, use the appropriate adapter (MongoDB, PostgreSQL, etc.).
