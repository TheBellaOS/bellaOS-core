# MongoDB Adapter for BellaOS

A robust MongoDB adapter for BellaOS that provides persistence, vector search capabilities, and caching functionality.
...
```bash
npm install @bellaos-plugins/adapter-mongodb
```

## Configuration

Add the adapter to your BellaOS configuration:

```json
{
  "plugins": ["@bellaos-plugins/adapter-mongodb"],
  "settings": {
    "MONGODB_CONNECTION_STRING": "your_mongodb_connection_string",
    "MONGODB_DATABASE": "your_database_name"  // Optional, defaults to "bellaAgent"
  }
}
```
