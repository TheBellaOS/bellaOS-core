...
Internet Computer Protocol (ICP) plugin for Bella OS.
...
```bash
pnpm install @bellaos/plugin-icp
```
...
```typescript
import { icpPlugin } from "@bellaos/plugin-icp";

// Register the plugin with Bella
bella.registerPlugin(icpPlugin);
```
