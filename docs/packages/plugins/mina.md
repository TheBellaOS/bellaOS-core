...
Core Mina blockchain plugin for Bella OS that provides essential services and actions for token operations and wallet management.
...
```bash
npm install @bellaos/plugin-mina
```
...
```typescript
import { minaPlugin } from "@bellaos/plugin-mina";
