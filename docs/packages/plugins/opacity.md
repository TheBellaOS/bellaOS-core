...
# @bellaos/adapter-opacity

This adapter integrates Opacity proofs into BellaOS, enabling verifiable inference results from various AI model providers available through the [CloudFlare AI Gateway](https://developers.cloudflare.com/ai-gateway). It implements the `IVerifiableInferenceAdapter` interface, making it compatible with other verifiable inference solutions.

## Installation

```bash
pnpm add @bellaos/adapter-opacity
```
...
```typescript
import { OpacityAdapter } from "@bellaos/adapter-opacity";
