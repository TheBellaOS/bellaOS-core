# @bellaos/plugin-cronos

Cronos plugin for Bella, extending the EVM plugin functionality.
...
```bash
pnpm add @bellaos/plugin-cronos
```
...
### Basic Setup
```typescript
import { cronosPlugin } from "@bellaos/plugin-cronos";

// Use the plugin in your Bella configuration
const config = {
...
1. **Basic Setup**
   ...
   - `plugins`: Include `@bellaos/plugin-cronos`
...
- All standard EVM functionality inherited from @bellaos/plugin-evm
