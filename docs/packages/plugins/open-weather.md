# @bellaos/plugin-open-weather

A plugin for Bella that enables weather checking using the OpenWeather API.
...
```bash
npm install @bellaos/plugin-open-weather
```
...
```typescript
import { openWeatherPlugin } from "@bellaos/plugin-open-weather";
