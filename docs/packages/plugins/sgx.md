# SGX Plugin for Bella

The SGX Plugin for Bella enhances the platform by providing Intel SGX attestation capabilities.
...
After starting Bella, the provider `sgxAttestationProvider` will be registered into Bella through plugin-sgx. The environment variable `SGX` is required to be set to `1` to enable plugin-sgx. And the `SGX` is always set to `1` in the SGX environment through the `bella.manifest.template` file.

When Bella starts, the `sgxAttestationProvider` will generate SGX attestation in each request. And you can use the `SgxAttestationProvider` to generate SGX remote attestations for your own plugins / clients.
...
```bash
sudo docker run -it --name bella_sgx \
    --mount type=bind,source={your_bella_path},target=/root/bella \
    --device /dev/sgx/enclave \
    --device /dev/sgx/provision \
    gramineproject/gramine:stable-jammy
```

After entering the docker, you can use the following command to prepare the Bella environment:
...
# Gramine will utilize this path as the default Node.js location to run Bella.
