# TEE Log Plugin for Bella

The TEE Log Plugin for Bella is designed to enhance the logging capabilities of the Bella by providing a structured way to generate, store and verify TEE (Trusted Execution Environment) logs for agents. This plugin ensures that all sensitive interactions are securely logged, providing a transparent and tamper-resistant record of all sensitive activities.

## Background

As Bella is a fully autonomous AI agent capable of running within a TEE, we need to demonstrate to the outside world that we are indeed operating within a TEE. This allows external parties to verify that our actions are protected by the TEE and that they are autonomously executed by Bella, without any third-party interference. Therefore, it is necessary to leverage TEE's remote attestation and establish a TEE logging mechanism to prove that these operations are entirely and autonomously performed by Bella within the TEE.
...
- using Intel SGX (Gramine), you need to enable the plugin-sgx in the Bella runtime, which is enabled in SGX env automatically.
- using Intel TDX (dstack), you need to enable the plugin-tee in the Bella runtime.
...
Enable the TEE logging in the Bella .env file:
...
```json
"@bellaos/plugin-tee-log": "workspace:*",
```
...
After configuring the logging for the action, you can run the Bella and see the logs through the client-direct REST API. 
