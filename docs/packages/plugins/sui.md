...
Core Sui blockchain plugin for Bella OS that provides essential services and actions for token operations and wallet management.
...
```bash
npm install @bellaos/plugin-sui
```
...
```typescript
import { suiPlugin } from "@bellaos/plugin-sui";
