# @bellaos/plugin-zapper

A plugin for Bella that allows users to fetch portfolio data using the Zapper API.
...
```bash
npm install @bellaos/plugin-zapper
```
...
```typescript
import { zapperPlugin } from "@bellaos/plugin-zapper";
