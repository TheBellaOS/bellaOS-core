# @bellaos/plugin-3d-generation

A plugin for generating 3D models using the FAL.ai API within the BellaOS ecosystem.
...
```bash
pnpm install @bellaos/plugin-3d-generation
```
...
```typescript
import { ThreeDGenerationPlugin } from "@bellaos/plugin-3d-generation";
```
