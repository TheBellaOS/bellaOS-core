...
Core Solana blockchain plugin for Bella OS that provides essential services and actions for token operations, trading, and DeFi integrations.

## Overview

The Solana plugin serves as a foundational component of Bella OS, bridging Solana blockchain capabilities with the Bella ecosystem. It provides crucial services for token operations, trading, portfolio management, and DeFi integrations, enabling both automated and user-directed interactions with the Solana blockchain.
...
```bash
npm install @bellaos/plugin-solana
```
...
```typescript
import { solanaPlugin } from "@bellaos/plugin-solana";
