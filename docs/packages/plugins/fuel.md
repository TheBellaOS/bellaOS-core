...
A plugin for interacting with the Fuel blockchain within the BellaOS ecosystem.
...
```bash
pnpm install @bellaos/plugin-fuel
```
...
```typescript
import { fuelPlugin } from "@bellaos/plugin-fuel";
```
