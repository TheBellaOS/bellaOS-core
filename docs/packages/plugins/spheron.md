# Spheron Protocol Plugin for Bella

This plugin integrates the Spheron Protocol SDK into the Bella ecosystem, providing functionality for managing deployments, escrow operations, and lease management.
...
```bash
npm install @bellaos/plugin-spheron
```
...
```typescript
import { spheronPlugin } from "@bellaos/plugin-spheron";

// Register with Bella
bella.registerPlugin(spheronPlugin);
```
...
1. You can ask the bella to deploy a template like `jupyter-notebook`, `vscode`, `ollama`, or `heurist-miner` for you with just natural language.
2. You can also customize the deployment with natural language.
3. You can also ask the bella to close the deployment.
4. You can also ask the bella to check the balance of your account.
5. You can also ask the bella to deposit or withdraw tokens from your account.
