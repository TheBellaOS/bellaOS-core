# @bellaos/plugin-avail - Plugin for Avail

This is a plugin for using Bella to interact with the Avail DA network. Defaults to Turing testnet, but can be customized to use Mainnet by changing the RPC in the `.env` file at `AVAIL_RPC_URL`.
