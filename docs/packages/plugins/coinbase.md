# @bellaos/plugin-coinbase

A comprehensive Coinbase integration plugin for BellaOS that provides access to Coinbase's various APIs and services.
...
```bash
npm install @bellaos/plugin-coinbase
```
...
```typescript
import { plugins } from "@bellaos/plugin-coinbase";
...
- `@bellaos/core`: Core BellaOS functionality
