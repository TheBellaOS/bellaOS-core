...
const REGISTRY_URL = 'https://raw.githubusercontent.com/bellaos-plugins/registry/refs/heads/main/index.json';
...
function processRegistryEntry(name, repoUrl) {
  ...
  const displayName = name
    ...
    .replace('@bellaos-plugins/client-', '')
    .replace('@bellaos-plugins/adapter-', '')
    .replace(/-/g, ' ')
    .split(' ')
  ...
  for (const pluginId in customData) {
    if (!registryPluginNames.includes(pluginId)) {
      ...
      missingPlugins.push({
        ...
        repo_url: `github:bellaos-plugins/${pluginId.split('/').pop()}`,
        repo_path: `bellaos-plugins/${pluginId.split('/').pop()}`,
        display_name: displayName,
        type: type
      });
    }
