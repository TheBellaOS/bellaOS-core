...
# 🛡️ Deploying Bella to TEE with Oasis ROFL

![](/img/bella_in_tee.jpg)

## Overview

Bella can be deployed to a Trusted Execution Environment (TEE) using the [Oasis SDK](https://github.com/oasisprotocol/oasis-sdk). This guide will walk you through the process of setting up and running Bella in a TEE environment using the Oasis ROFL tool.
...
Bella's TEE implementation consists of several key components that work together to provide a secure execution environment:
...
- Secure execution of the Bella agent
...
Here's a comprehensive list of secrets you might want to set for your Bella agent:
...
echo -n "postgresql://user:password@localhost:5432/bella" | oasis rofl secret set POSTGRES_URL -
```
...
Before getting started with Bella in TEE, ensure you have:

- [Oasis CLI] installed
- Docker Desktop or Orbstack (Orbstack is recommended)
- Testnet tokens from the [Oasis Faucet](https://faucet.testnet.oasis.dev/)
- A character file for your Bella agent (you can use the example character file or create your own)
...
1. **Configure the ROFL app**
    ...
    services:
      bella:
        restart: always
        image: ghcr.io/bellaos/bella:latest
        build:
          context: .
...
Congratulations! You have successfully deployed Bella to a TEE environment.
