...
![](/img/bella_in_tee.jpg)

## Overview

The Bella agent can be deployed in a TEE environment to ensure the security and privacy of the agent's data. This guide will walk you through the process of setting up and running an Bella agent in a TEE environment by utilizing the TEE Plugin in the Bella Framework.

### Background

The TEE Plugin in the Bella Framework is built on top of the [Dstack SDK](https://github.com/Dstack-TEE/dstack), which is designed to simplify the steps for developers to deploy programs to CVM (Confidential VM), and to follow the security best practices by default. The main features include:
...
Bella's TEE implementation consists of two primary providers that handle secure key management operations and remote attestations.
...
Before getting started with Bella, ensure you have:
...
### Run an Bella Agent Locally with TEE Simulator

1. **Configure Bella Agent**

    Go through the [configuration guide](../guides/configuration.md) to set up your Bella agent.
...
4. **Verify TEE Attestation**
    ...
    You can verify the TEE attestation quote by going to the [TEE RA Explorer](https://proof.t16z.com/) and pasting the attestation quote from the agent logs. Here's an example of interacting with the Bella agent to ask for the agent's wallet address:
...
### Build, Test, and Publish an Bella Agent Docker Image

Now that we have run the Bella agent in the TEE simulator, we can build and publish an Bella agent Docker image to prepare for deployment to a real TEE environment.
...
docker build -t username/bella-agent:latest .

# For architecture other than AMD64, run
docker buildx build --platform=linux/amd64 -t username/bella-agent:latest .
```

For Linux/AMD64 machines, you can now test the agent locally by updating the `TEE_MODE` environment variable to `DOCKER` and setting the environment variables in the [docker-compose.yaml](https://github.com/bellaos/bella/blob/main/docker-compose.yaml) file. Once you have done that, you can start the agent by running:
...
```bash
docker push username/bella-agent:latest
```

Now we are ready to deploy the Bella agent to a real TEE environment.

### Run an Bella Agent in a Real TEE Environment

Before deploying the Bella agent to a real TEE environment, you need to create a new TEE account on the [TEE Cloud](https://cloud.phala.network/login). Reach out to Phala Network on [Discord](https://discord.gg/phalanetwork) if you need help.
...
services:
    tee:
        ...
        image: username/bella-agent:latest
        stdin_open: true
        tty: true
        volumes:
...
Now you can deploy the Bella agent to a real TEE environment. Go to the [TEE Cloud](https://cloud.phala.network/login) and click on the `Create VM` button to configure your Bella agent deployment.
...
Finally, click on the `Submit` button to deploy your Bella agent.

This will take a few minutes to complete. Once the deployment is complete, you can click on the `View` button to view your Bella agent.
...
Congratulations! You have successfully run an Bella agent in a real TEE environment.
