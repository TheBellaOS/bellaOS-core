# Bella - framework de simulação Multi-agentes

# https://github.com/bellaOS/bella

# Visite https://bella.builders para suporte
...
- Altamente extensível - crie suas próprias ações e clientes para ampliar as capacidades do Bella
...
Para evitar conflitos no diretório principal, recomendamos adicionar ações personalizadas a um diretório chamado  `custom_actions` e, em seguida, incluí-las no arquivo `bellaConfig.yaml`. Consulte o arquivo `bellaConfig.example.yaml` para um exemplo.
...
O servidor do Docusaurus será iniciado e você poderá verificar a documentação localmente em https://localhost:3000/bella.
