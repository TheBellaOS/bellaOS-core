...
Bella's plugin system provides a modular way to extend the core functionality with additional features, actions, evaluators, and providers. Plugins are self-contained modules that can be easily added or removed to customize your agent's capabilities.

## Core Plugin Concepts

### Plugin Structure

Each plugin in Bella must implement the `Plugin` interface with the following properties:
...
```bash
pnpm add @bellaos/plugin-[name]
```

2. Import and register the plugin in your character configuration:

```typescript
import { bootstrapPlugin } from "@bella/plugin-bootstrap";
import { imageGenerationPlugin } from "@bella/plugin-image-generation";
import { buttplugPlugin } from "@bella/plugin-buttplug";
const character = {
...
#### 1. Bootstrap Plugin (`@bella/plugin-bootstrap`)
...
#### 2. Image Generation Plugin (`@bella/plugin-image-generation`)
...
#### 3. Node Plugin (`@bella/plugin-node`)
...
#### 4. Solana Plugin (`@bella/plugin-solana`)
...
#### 5. Coinbase Commerce Plugin (`@bella/plugin-coinbase`)
...
**Description:**
This plugin enables Bella to interact with the Coinbase Commerce API to create and manage payment charges, providing seamless integration with cryptocurrency-based payment systems.
...
#### 6. Coinbase MassPayments Plugin (`@bella/plugin-coinbase`)
...
1. **Configure the Plugin**
   Add the plugin to your character's configuration:
    ...
    ```typescript
    import { coinbaseMassPaymentsPlugin } from "@bella/plugin-coinbase-masspayments";
...
#### 7. Coinbase Token Contract Plugin (`@bella/plugin-coinbase`)
...
1. **Configure the Plugin**
   Add the plugin to your character's configuration:
    ...
    ```typescript
    import { tokenContractPlugin } from "@bella/plugin-coinbase";
...
#### 8. TEE Plugin (`@bellaos/plugin-tee`)

Integrates [Dstack SDK](https://github.com/Dstack-TEE/dstack) to enable TEE (Trusted Execution Environment) functionality and deploy secure & privacy-enhanced Bella Agents:
...
#### 9. Webhook Plugin (`@bella/plugin-coinbase-webhooks`)
...
The Webhook Plugin enables Bella to interact with the Coinbase SDK to create and manage webhooks. This allows for real-time event handling and notifications based on specific criteria set by the user.
...
1. **Configure the Plugin**
   Add the plugin to your character’s configuration:
    ...
    ```typescript
    import { webhookPlugin } from "@bella/plugin-coinbase-webhooks";
...
#### 10. Fuel Plugin (`@bellaos/plugin-fuel`)
...
1. **Configure the Plugin**
   Add the plugin to your character's configuration:
    ...
    ```typescript
    import { fuelPlugin } from "@bella/plugin-fuel";
...
#### 11. Marlin TEE Plugin (`@bellaos/plugin-tee-marlin`)

Makes Bella TEE-aware by using the [Marlin Oyster](https://github.com/marlinprotocol/oyster-monorepo) platform tooling with the goal of making Bella agents verifiable and private.
...
- `REMOTE_ATTESTATION`: Lets Bella respond with a remote attestation that users can verify. Just ask Bella for an attestation! E.g. "Attest yourself", "Give me a remote attestation".
...
Just ask Bella for a remote attestation!
...
### 12. Allora Plugin (`@bellaos/allora-plugin`)

The [Allora Network](https://allora.network) plugin seamlessly empowers Bella agents with real-time, advanced, self-improving AI inferences, delivering high-performance insights without introducing any additional complexity.
...
1. Add the plugin to your character's configuration
    ...
    ```typescript
    import { alloraPlugin } from "@bella/plugin-allora";
...
### 13. Form Plugin (`@bellaos/plugin-form`)
...
```typescript
import { Plugin, Action, Evaluator, Provider } from "@bellaos/core";
