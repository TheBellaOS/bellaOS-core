...
Database Adapters provide Bella's persistence layer, enabling storage and retrieval of memories, relationships, goals, and other data through a unified interface. The system supports multiple database backends:

### Available Adapters

Each adapter is optimized for different use cases:

- **PostgreSQL** (`@bellaos/adapter-postgres`)

    - Production-ready with vector search
    - Connection pooling and high performance
    - JSONB and pgvector support

- **SQLite** (`@bellaos/adapter-sqlite`)

    - Lightweight local development
    - No external dependencies
    - Full-text search capabilities

- **Supabase** (`@bellaos/adapter-supabase`)

    - Cloud-native PostgreSQL
    - Real-time subscriptions
    - Built-in RPC functions

- **SQL.js** (`@bellaos/adapter-sqljs`)
...
Bella's database adapters provide a unified interface for data persistence across different storage backends. The following diagram shows how adapters integrate with the system:
...
```bash
# PostgreSQL
pnpm add @bellaos/adapter-postgres pg

# SQLite
pnpm add @bellaos/adapter-sqlite better-sqlite3

# SQL.js
pnpm add @bellaos/adapter-sqljs sql.js

# Supabase
pnpm add @bellaos/adapter-supabase @supabase/supabase-js

# PgLite
pnpm add @bellaos/adapter-pglite @electric-sql/pglite
```
...
```typescript
import { PostgresDatabaseAdapter } from "@bellaos/adapter-postgres";
