...
Database Adapters provide the persistence layer for Bella, enabling storage and retrieval of memories, relationships, goals, and other core data. The system supports multiple database backends through a unified interface.

## Available Adapters

Bella includes the following database adapters:

- **PostgreSQL Adapter** (`@bella/adapter-postgres`) - Production-ready adapter for PostgreSQL databases
- **SQLite Adapter** (`@bella/adapter-sqlite`) - Lightweight adapter for SQLite, perfect for development
- **SQL.js Adapter** (`@bella/adapter-sqljs`) - In-memory SQLite adapter for testing
- **Supabase Adapter** (`@bella/adapter-supabase`) - Cloud-native adapter for Supabase

## Installation

```bash
# PostgreSQL
pnpm add @bella/adapter-postgres

# SQLite
pnpm add @bella/adapter-sqlite

# SQL.js
pnpm add @bella/adapter-sqljs

# Supabase
pnpm add @bella/adapter-supabase
```
...
```typescript
import { SqliteDatabaseAdapter } from "@bella/adapter-sqlite";
...
```typescript
import { PostgresDatabaseAdapter } from "@bella/adapter-postgres";
...
```typescript
import { SupabaseDatabaseAdapter } from "@bella/adapter-supabase";
...
Memories are the fundamental unit of storage in Bella. They represent messages, documents, and other content with optional embeddings for semantic search.
