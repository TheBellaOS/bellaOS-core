...
// import type { TeeLogQuery, TeeLogService } from "@bellaos/plugin-tee-log";
// import { REST, Routes } from "discord.js";
import type { DirectClient } from ".";
import { validateUuid } from "@bellaos/core";
...
export function createApiRouter(
    ...
    router.post("/agents/:agentId/set", async (req, res) => {
        ...
        try {
            ...
            bellaLogger.error(`Error parsing character: ${e}`);
            res.status(400).json({
                success: false,
                message: e.message,
            });
        ...
        try {
            ...
            bellaLogger.log(`${character.name} started`);
        } catch (e) {
            bellaLogger.error(`Error starting agent: ${e}`);
            res.status(500).json({
                success: false,
                message: e.message,
            });
    ...
    //     } catch (error) {
    //         bellaLogger.error("Failed to get TEE agents:", error);
    //         res.status(500).json({
    ...
    //     } catch (error) {
    //         bellaLogger.error("Failed to get TEE agent:", error);
    //         res.status(500).json({
    ...
    //             });
    //         } catch (error) {
    //             bellaLogger.error("Failed to get TEE logs:", error);
    //             res.status(500).json({
