...
export class DirectClient {
    ...
    constructor() {
        bellaLogger.log("DirectClient constructor");
        this.app = express();
        this.app.use(cors());
        this.agents = new Map();
        ...
        this.app.get(
            ...
            async (req: express.Request, res: express.Response) => {
                ...
                bellaLogger.log("Download directory:", downloadDir);

                try {
                    bellaLogger.log("Creating directory...");
                    await fs.promises.mkdir(downloadDir, { recursive: true });

                    bellaLogger.log("Fetching file...");
                    ...
                    bellaLogger.log("Response headers:", fileResponse.headers);
                    ...
                    bellaLogger.log("Saving as:", fileName);

                    const arrayBuffer = await fileResponse.arrayBuffer();
                    const buffer = Buffer.from(arrayBuffer);

                    const filePath = path.join(downloadDir, fileName);
                    bellaLogger.log("Full file path:", filePath);
                    ...
                    bellaLogger.error("Detailed error:", error);
                    res.status(500).json({
    ...
    public start(port: number) {
        ...
        const gracefulShutdown = () => {
            bellaLogger.log("Received shutdown signal, closing server...");
            this.server.close(() => {
                bellaLogger.success("Server closed successfully");
                process.exit(0);
            });
    ...
    public async stop() {
        if (this.server) {
            this.server.close(() => {
                bellaLogger.success("Server stopped");
            });
        }
    }
}
...
export const DirectClientInterface: Client = {
    ...
    start: async (_runtime: IAgentRuntime) => {
        bellaLogger.log("DirectClientInterface start");
