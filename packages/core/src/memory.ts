...
export class MemoryManager implements IMemoryManager {
    ...
    async addEmbeddingToMemory(memory: Memory): Promise<Memory> {
        ...
        try {
            ...
            bellaLogger.error("Failed to generate embedding:", error);
    ...
    async createMemory(memory: Memory, unique = false): Promise<void> {
        ...
        if (existingMessage) {
            bellaLogger.debug("Memory already exists, skipping");
            return;
        }

        bellaLogger.log("Creating Memory", memory.id, memory.content.text);
