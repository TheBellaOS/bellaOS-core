...
async function get(
    ...
    if (!message?.content?.text) {
        bellaLogger.warn("Invalid message for knowledge query:", {
            message,
    ...
    bellaLogger.debug("Knowledge query:", {
    ...
    if (!processed || processed.trim().length === 0) {
        bellaLogger.warn("Empty processed text for knowledge query");
        return [];
    }
...
export function preprocess(content: string): string {
    bellaLogger.debug("Preprocessing text:", {
        input: content,
