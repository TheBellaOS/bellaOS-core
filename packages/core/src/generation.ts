...
import { bellaLogger } from "./index.ts";
import {
    models,
    getModelSettings,
...
export async function trimTokens(
    ...
    bellaLogger.warn(`Unsupported tokenizer type: ${tokenizerType}`);
    return truncateTiktoken("gpt-4o", context, maxTokens);
}
...
//     } catch (error) {
//         bellaLogger.error("Error in trimTokens:", error);
...
async function getOnChainEternalAISystemPrompt(
    ...
    if (agentId && providerUrl && contractAddress) {
        ...
        try {
            ...
            if (result) {
                bellaLogger.info("on-chain system-prompt response", result[0]);
                const value = result[0].toString().replace("0x", "");
                const content = Buffer.from(value, "hex").toString("utf-8");
                bellaLogger.info("on-chain system-prompt", content);
                return await fetchEternalAISystemPrompt(runtime, content);
            } else {
                return undefined;
            }
        } catch (error) {
            bellaLogger.error(error);
            bellaLogger.error("err", error);
        }
    }
    return undefined;
}
...
async function fetchEternalAISystemPrompt(
    ...
    if (containsSubstring) {
        ...
        bellaLogger.info("fetch lightHouse", lightHouse);
        const responseLH = await fetch(lightHouse, {
            method: "GET",
        });
        bellaLogger.info("fetch lightHouse resp", responseLH);
        if (responseLH.ok) {
            ...
            bellaLogger.info("fetch gcs", gcs);
            const responseGCS = await fetch(gcs, {
                method: "GET",
            });
            bellaLogger.info("fetch lightHouse gcs", responseGCS);
            if (responseGCS.ok) {
...
function getCloudflareGatewayBaseURL(
    ...
    bellaLogger.debug("Cloudflare Gateway Configuration:", {
    ...
    if (!isCloudflareEnabled) {
        bellaLogger.debug("Cloudflare Gateway is not enabled");
        return undefined;
    }
    ...
    bellaLogger.info("Using Cloudflare Gateway:", {
        provider,
        baseURL,
...
// verifiableInferenceOptions,
{
    ...
    bellaLogger.log("Generating text...");

    bellaLogger.info("Generating text with options:", {
        modelProvider: runtime.modelProvider,
        model: modelClass,
        // verifiableInference,
    });
    bellaLogger.log("Using provider:", runtime.modelProvider);
    ...
    //                 verifiableInferenceOptions
    //             );
    //         bellaLogger.log("Verifiable inference result:", result);
    //         // Verify the proof
    //         const isValid =
    ...
    //         return result.text;
    //     } catch (error) {
    //         bellaLogger.error("Error in verifiable inference:", error);
    //         throw error;
    //     }
    // }

    const provider = runtime.modelProvider;
    bellaLogger.debug("Provider settings:", {
        provider,
        hasRuntime: !!runtime,
        runtimeSettings: {
    ...
    bellaLogger.info("Selected model:", model);
    ...
    try {
        ...
        switch (provider) {
            ...
            case ModelProviderName.KLUSTERAI: {
                ...
                //bellaLogger.debug("OpenAI baseURL result:", { baseURL });
            ...
            case ModelProviderName.ETERNALAI: {
                bellaLogger.debug("Initializing EternalAI model.");
                const openai = createOpenAI({
                    apiKey,
                    baseURL: endpoint,
                    fetch: runtime.fetch,
                ...
                response = openaiResponse;
                bellaLogger.debug("Received response from EternalAI model.");
                break;
            }
            ...
            case ModelProviderName.GOOGLE: {
                ...
                bellaLogger.debug("Received response from Google model.");
                break;
            }
            ...
            case ModelProviderName.MISTRAL: {
                ...
                bellaLogger.debug("Received response from Mistral model.");
                break;
            }
            ...
            case ModelProviderName.ANTHROPIC: {
                ...
                bellaLogger.debug("Anthropic baseURL result:", { baseURL });
                ...
                bellaLogger.debug("Received response from Anthropic model.");
                break;
            }

            case ModelProviderName.CLAUDE_VERTEX: {
                bellaLogger.debug("Initializing Claude Vertex model.");
            ...
            case ModelProviderName.GROK: {
                bellaLogger.debug("Initializing Grok model.");
                ...
                bellaLogger.debug("Received response from Grok model.");
                break;
            }
            ...
            case ModelProviderName.MEM0: {
                ...
                const mem0 = createMem0({
                    ...
                    mem0Config: {
                        user_id: runtime.getSetting("MEM0_USER_ID") || "bella-os-user"
                    }
                });
                ...
                bellaLogger.debug("Received response from Mem0 Provider.");
                break;
            }
            ...
            case ModelProviderName.GROQ: {
                ...
                bellaLogger.debug("Groq baseURL result:", { baseURL });
                ...
                bellaLogger.debug("Received response from Groq model.");
                break;
            }
            ...
            case ModelProviderName.LLAMALOCAL: {
                ...
                bellaLogger.debug("Received response from local Llama model.");
                break;
            }

            case ModelProviderName.REDPILL: {
                bellaLogger.debug("Initializing RedPill model.");
                ...
                bellaLogger.debug("Received response from redpill model.");
                break;
            }

            case ModelProviderName.OPENROUTER: {
                bellaLogger.debug("Initializing OpenRouter model.");
                const serverUrl = getEndpoint(provider);
                ...
                bellaLogger.debug("Received response from OpenRouter model.");
                break;
            }

            case ModelProviderName.OLLAMA:
                {
                    bellaLogger.debug("Initializing Ollama model.");
                ...
                bellaLogger.debug("Received response from Ollama model.");
                break;

            case ModelProviderName.HEURIST: {
                bellaLogger.debug("Initializing Heurist model.");
                ...
                bellaLogger.debug("Received response from Heurist model.");
                break;
            }
            case ModelProviderName.GAIANET: {
                bellaLogger.debug("Initializing GAIANET model.");
                ...
                bellaLogger.debug("Using GAIANET model with baseURL:", baseURL);
                ...
                response = openaiResponse;
                bellaLogger.debug("Received response from GAIANET model.");
                break;
            }

            case ModelProviderName.ATOMA: {
                bellaLogger.debug("Initializing Atoma model.");
                ...
                bellaLogger.debug("Received response from Atoma model.");
                break;
            }

            case ModelProviderName.GALADRIEL: {
                bellaLogger.debug("Initializing Galadriel model.");
                const headers = {};
                ...
                bellaLogger.debug("Received response from Galadriel model.");
                break;
            }

            case ModelProviderName.INFERA: {
                bellaLogger.debug("Initializing Infera model.");
                ...
                bellaLogger.debug("Received response from Infera model.");
                break;
            }

            case ModelProviderName.VENICE: {
                bellaLogger.debug("Initializing Venice model.");
                ...
                bellaLogger.debug("Received response from Venice model.");
                break;
            }

            case ModelProviderName.NVIDIA: {
                bellaLogger.debug("Initializing NVIDIA model.");
                ...
                bellaLogger.debug("Received response from NVIDIA model.");
                break;
            }

            case ModelProviderName.DEEPSEEK: {
                bellaLogger.debug("Initializing Deepseek model.");
                ...
                bellaLogger.debug("Received response from Deepseek model.");
                break;
            }

            case ModelProviderName.LIVEPEER: {
                bellaLogger.debug("Initializing Livepeer model.");
                ...
                const fetchResponse = await runtime.fetch(endpoint + "/llm", {
                    ...
                    headers: {
                        ...
                        "Content-Type": "application/json",
                        Authorization: "Bearer bella-app-llm",
                    },
            ...
            case ModelProviderName.SECRETAI:
                {
                    bellaLogger.debug("Initializing SecretAI model.");
            ...
            case ModelProviderName.BEDROCK: {
                bellaLogger.debug("Initializing Bedrock model.");
                ...
                bellaLogger.debug("Received response from Bedrock model.");
                break;
            }

            default: {
                const errorMessage = `Unsupported provider: ${provider}`;
                bellaLogger.error(errorMessage);
        ...
        bellaLogger.error("Error in generateText:", error);
        throw error;
    }
}
...
export async function generateShouldRespond({
    ...
    while (true) {
        try {
            ...
            bellaLogger.debug("Received response from generateText:", response);
            const parsedResponse = parseShouldRespondFromText(response.trim());
            if (parsedResponse) {
                bellaLogger.debug("Parsed response:", parsedResponse);
                return parsedResponse;
            } else {
                bellaLogger.debug("generateShouldRespond no response");
            }
        } catch (error) {
            bellaLogger.error("Error in generateShouldRespond:", error);
            if (
        ...
        bellaLogger.log(`Retrying in ${retryDelay}ms...`);
...
export async function splitChunks(
    ...
    bellaLogger.debug(`[splitChunks] Starting text split`);
    ...
    if (bleed < 0) {
        bellaLogger.warn(`Invalid bleed (${bleed}), using default 100`);
        bleed = 100;
    }

    const chunks = splitText(content, chunkSize, bleed);

    bellaLogger.debug(`[splitChunks] Split complete:`, {
...
export async function generateTrueOrFalse({
    ...
    while (true) {
        try {
            ...
            bellaLogger.error("Error in generateTrueOrFalse:", error);
        }
...
export async function generateTextArray({
    ...
    if (!context) {
        bellaLogger.error("generateTextArray context is empty");
        return [];
    }
    let retryDelay = 1000;
    ...
    while (true) {
        try {
            ...
            bellaLogger.error("Error in generateTextArray:", error);
        }
...
export async function generateObjectDeprecated({
    ...
    if (!context) {
        bellaLogger.error("generateObjectDeprecated context is empty");
        return null;
    }
    let retryDelay = 1000;
    ...
    while (true) {
        try {
            ...
            bellaLogger.error("Error in generateObject:", error);
        }
...
export async function generateObjectArray({
    ...
    if (!context) {
        bellaLogger.error("generateObjectArray context is empty");
        return [];
    }
    let retryDelay = 1000;
...
export async function generateMessageResponse({
    ...
    context = await trimTokens(context, max_context_length, runtime);
    bellaLogger.debug("Context:", context);
    let retryLength = 1000; // exponential backoff
    while (true) {
        try {
            bellaLogger.log("Generating message response..");
            ...
            if (!parsedContent) {
                bellaLogger.debug("parsedContent is null, retrying");
                continue;
            }

            return parsedContent;
        } catch (error) {
            bellaLogger.error("ERROR:", error);
            // wait for 2 seconds
            retryLength *= 2;
            await new Promise((resolve) => setTimeout(resolve, retryLength));
            bellaLogger.debug("Retrying...");
        }
    }
}
...
export const generateImage = async (
    ...
    bellaLogger.info("Generating image with options:", {
        ...
        if (runtime.imageModelProvider === ModelProviderName.HEURIST) {
            ...
            const base64s = await Promise.all(
                togetherResponse.data.map(async (image) => {
                    if (!image.url) {
                        bellaLogger.error("Missing URL in image data:", image);
            ...
            bellaLogger.debug(`Generated ${base64s.length} images`);
            ...
            const result = await fal.subscribe(model, {
                ...
                onQueueUpdate: (update) => {
                    if (update.status === "IN_PROGRESS") {
                        bellaLogger.info(update.logs.map((log) => log.message));
                    }
                },
            });
            ...
            try {
                ...
                const response = await fetch(
                    `${baseUrl.toString()}text-to-image`,
                    {
                        ...
                        headers: {
                            "Content-Type": "application/json",
                            Authorization: "Bearer bella-app-img",
                        },
                        body: JSON.stringify({
                            model_id:
...
async function handleAnthropic({
    ...
    bellaLogger.debug("Handling Anthropic request with Cloudflare check");
    if (mode === "json") {
        bellaLogger.warn("Anthropic mode is set to json, changing to auto");
        mode = "auto";
    }
    const baseURL = getCloudflareGatewayBaseURL(runtime, "anthropic");
    bellaLogger.debug("Anthropic handleAnthropic baseURL:", { baseURL });
...
async function handleGroq({
    ...
    bellaLogger.debug("Handling Groq request with Cloudflare check");
    const baseURL = getCloudflareGatewayBaseURL(runtime, "groq");
    bellaLogger.debug("Groq handleGroq baseURL:", { baseURL });
...
export async function generateTweetActions({
    ...
    while (true) {
        try {
            ...
            if (actions) {
                bellaLogger.debug("Parsed tweet actions:", actions);
                return actions;
            } else {
                bellaLogger.debug("generateTweetActions no valid response");
            }
        } catch (error) {
            bellaLogger.error("Error in generateTweetActions:", error);
            if (
                error instanceof TypeError &&
                error.message.includes("queueTextCompletion")
            ) {
