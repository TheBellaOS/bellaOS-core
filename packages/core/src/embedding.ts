...
export const getEmbeddingConfig = (): EmbeddingConfig => {
    ...
    if (settings.USE_CUSTOM_EMBEDDING?.toLowerCase() === "true") {
        ...
        if (!settings.CUSTOM_EMBEDDING_DIMENSIONS || !settings.CUSTOM_EMBEDDING_MODEL || 
            ...
            bellaLogger.error("Custom embedding configuration error", {
...
async function getRemoteEmbedding(
    ...
    try {
        ...
        if (!response.ok) {
            bellaLogger.error("API Response:", await response.text()); // Debug log
            throw new Error(
        ...
        bellaLogger.error("Full error details:", e);
        throw e;
    }
}
...
export async function embed(runtime: IAgentRuntime, input: string): Promise<number[]> {
    bellaLogger.debug("Embedding request:", {
    ...
    if (!input || typeof input !== "string" || input.trim().length === 0) {
        bellaLogger.warn("Invalid embedding input:", {
            input,
            type: typeof input,
            length: input?.length,
        });
    ...
    async function retrieveCachedEmbedding(
        ...
        if (!input) {
            bellaLogger.log("No input to retrieve cached embedding for");
            return null;
        }
