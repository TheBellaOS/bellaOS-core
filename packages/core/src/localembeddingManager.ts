...
import bellaLogger from "./logger";
...
class LocalEmbeddingModelManager {
    ...
    private async getRootPath(): Promise<string> {
        ...
        return rootPath.includes("/bella/")
            ? rootPath.split("/bella/")[0] + "/bella/"
    ...
    private async initializeModel(): Promise<void> {
        ...
        try {
            ...
            bellaLogger.debug("Initializing BGE embedding model...");
            ...
            bellaLogger.debug("BGE model initialized successfully");
        } catch (error) {
            bellaLogger.error("Failed to initialize BGE model:", error);
            throw error;
        }
    }
    ...
    public async generateEmbedding(input: string): Promise<number[]> {
        ...
        try {
            ...
            // bellaLogger.debug("Raw embedding from BGE:", {
            ...
            bellaLogger.error("Embedding generation failed:", error);
            throw error;
        }
    }
