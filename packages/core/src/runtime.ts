...
export class AgentRuntime implements IAgentRuntime {
    ...
    getService<T extends Service>(service: ServiceType): T | null {
        ...
        if (!serviceInstance) {
            bellaLogger.error(`Service ${service} not found`);
            return null;
        }
    ...
    async registerService(service: Service): Promise<void> {
        ...
        bellaLogger.log(`${this.character.name}(${this.agentId}) - Registering service:`, serviceType);
        ...
        bellaLogger.success(`${this.character.name}(${this.agentId}) - Service ${serviceType} registered successfully`);
    }
    ...
    constructor(opts: {
        ...
        bellaLogger.info(`${this.character.name}(${this.agentId}) - Initializing AgentRuntime with options:`, {
        ...
        bellaLogger.success(`Agent ID: ${this.agentId}`);
        ...
        bellaLogger.info(`${this.character.name}(${this.agentId}) - Setting Model Provider:`, {
        ...
        if (!Object.values(ModelProviderName).includes(this.modelProvider)) {
            bellaLogger.error("Invalid model provider:", this.modelProvider);
            bellaLogger.error(
                "Available providers:",
        ...
        if (!this.serverUrl) {
            bellaLogger.warn("No serverUrl provided, defaulting to localhost");
        }
    ...
    async initialize() {
        ...
        */
        ...
        if (
            ...
            bellaLogger.info(`[RAG Cleanup] Cleanup complete`);
        }
    }

    async stop() {
        bellaLogger.debug("runtime::stop - character", this.character.name);
    ...
    private async processCharacterKnowledge(items: string[]) {
        ...
        bellaLogger.info('discovered ' + toAdd.length + ' new knowledge items')
        const chunkSize = 512;
        const ps = [];
        ...
        bellaLogger.success(this.character.name, 'knowledge is synchronized');
    }
            ...
            try {
                ...
                if (
                    ...
                    try {
                        ...
                        bellaLogger.debug("[RAG Query Result]", {
                            relativePath: contentItem,
                            fullPath: filePath,
                            knowledgeId,
                            isShared,
    ...
    private async processCharacterRAGDirectory(dirConfig: {
        ...
        if (!dirConfig.directory) {
            bellaLogger.error("[RAG Directory] No directory specified");
            return;
        }
        ...
        try {
            ...
            bellaLogger.debug(`[RAG Directory] Searching in: ${dirPath}`);
    ...
    registerAction(action: Action) {
        bellaLogger.success(`${this.character.name}(${this.agentId}) - Registering action: ${action.name}`);
    ...
    async processActions(
        ...
        for (const response of responses) {
            if (!response.content?.action) {
                bellaLogger.warn("No action found in the response content.");
                continue;
            }
            ...
            bellaLogger.success(`Normalized action: ${normalizedAction}`);
            ...
            if (!action) {
                bellaLogger.info("Attempting to find action in similes.");
            ...
            if (!action.handler) {
                bellaLogger.error(`Action ${action.name} has no handler.`);
                continue;
            }
    ...
    async evaluate(
        ...
        const evaluatorPromises = this.evaluators.map(
            async (evaluator: Evaluator) => {
                bellaLogger.log("Evaluating", evaluator.name);
    ...
    async ensureUserExists(
        ...
        if (!account) {
            ...
            bellaLogger.success(`User ${userName} created successfully.`);
        }
    }
    ...
    async ensureRoomExists(roomId: UUID) {
        ...
        if (!room) {
            ...
            bellaLogger.log(`Room ${roomId} created successfully.`);
        }
    }
