...
import bellaLogger from "./logger.ts";
import {
    type IAgentRuntime,
...
export class RAGKnowledgeManager implements IRAGKnowledgeManager {
    ...
    private preprocess(content: string): string {
        if (!content || typeof content !== "string") {
            bellaLogger.warn("Invalid input for preprocessing");
            return "";
        }
    ...
    private hasProximityMatch(text: string, terms: string[]): boolean {
        ...
        for (let i = 0; i < allPositions.length - 1; i++) {
            if (Math.abs(allPositions[i] - allPositions[i + 1]) <= 5) {
                bellaLogger.debug("[Proximity Match]", {
                    terms,
    ...
    async createKnowledge(item: RAGKnowledgeItem): Promise<void> {
        if (!item.content.text) {
            bellaLogger.warn("Empty content in knowledge item");
            return;
        }
        ...
        try {
            ...
            bellaLogger.error(`Error processing knowledge ${item.id}:`, error);
            throw error;
        }
    }
    ...
    async cleanupDeletedKnowledgeFiles() {
        try {
            ...
            bellaLogger.debug("[Cleanup] Finished knowledge cleanup process");
        } catch (error) {
            bellaLogger.error(
    ...
    async processFile(file: {
        ...
        const timeMarker = (label: string) => {
            ...
            bellaLogger.info(`[Timing] ${label}: ${time.toFixed(2)}s`);
        };
        ...
        try {
            ...
            bellaLogger.info(`Generated ${totalChunks} chunks`);
            ...
            bellaLogger.error(`Error processing file ${file.path}:`, error);
            throw error;
        }
    }
}
