...
const pluginPkgPath = (pluginRepo) => {
  ...
  const bellaOSroot = pathUtil.resolve(__dirname, '../..')
  const pkgPath = bellaOSroot + '/packages/' + parts[1]
  return pkgPath
}

const packagedPlugins = ['cli', '@bellaos/client-direct', '@bellaos/core',
  '@bellaos/core-plugin-v1', 'dynamic-imports', '@bellaos/plugin-bootstrap']
...
program
  .name('bellaos')
  .description('bellaOS CLI - Manage your plugins')
  .version(version);

const pluginsCmd = new Command()
  .name('plugins')
  .description('manage bellaOS plugins')

let metadata = {}

async function getPlugins() {
  const resp = await fetch('https://raw.githubusercontent.com/bellaos-plugins/registry/refs/heads/main/index.json')
  const mostlyJson = await resp.text();
  //const resp = await axios.get('https://raw.githubusercontent.com/bellaos-plugins/registry/refs/heads/main/index.json')
...
pluginsCmd
  ...
  .action(async (opts) => {
    try {
      ...
      const bellaOSroot = pathUtil.resolve(__dirname, '../..')
      const installled = getInstalledPackages(bellaOSroot + '/packages')
...
pluginsCmd
  ...
  .action(async (plugin, opts) => {
    ...
    if (namePart === 'plugin-trustdb') {
      repo = 'bellaos-plugins/plugin-trustdb'
    } else {
    ...
    const pkgPath = bellaOSroot + '/packages/' + namePart
    ...
    const updateDependencies = (deps) => {
      ...
      let changed = false
      const okPackages = ['@bellaos/client-direct', '@bellaos/core', '@bellaos/core-plugin-v1', '@bellaos/plugin-bootstrap']
      for (const dep in deps) {
        if (okPackages.indexOf(dep) !== -1) continue // skip these, they're fine
        // do we want/need to perserve local packages like core?
        if (dep.startsWith("@bellaos/")) {
          const newDep = dep.replace("@bellaos/", "@bellaos-plugins/")
    ...
    // normalize @bellaos => @bellaos-plugins
    ...
    */
    const swapCoreDependencies = (deps) => {
      ...
      delete deps['@bellaos/core']
      deps['@bellaos/core-plugin-v1'] = 'workspace:*'
      fs.writeFileSync(packageJsonPath, JSON.stringify(packageJson, null, 2) + "\n")
    }
    ...
    for(const d in packageJson.dependencies) {
      ...
      if (!installedPlugins.includes(d)) {
        ...
        if (Object.keys(plugins).includes(d)) {
          ...
          try {
            const pluginAddDepOutput = execSync('npx bellaos plugins add ' + d, { cwd: bellaOSroot, stdio: 'pipe' }).toString().trim();
    ...
    // add core to plugin
    // # pnpm add @bellaos/core@workspace:* --filter ./packages/client-twitter

    // ok this can be an issue if it's referencing a plugin it couldn't be
    console.log('Making sure plugin has access to @bellaos/core-plugin-v1')
    try {
      const pluginAddCoreOutput = execSync('pnpm add @bellaos/core-plugin-v1@workspace:* --filter ./packages/' + plugin, { cwd: bellaOSroot, stdio: 'pipe' }).toString().trim();
    } catch(e) {
    ...
    if (packageJson.name !== '@bellaos-plugins/' + namePart) {
      packageJson.name = '@bellaos-plugins/' + namePart
    ...
    console.log('installing NPMs', bellaOSroot)
    try {
      const projectInstallOutput = execSync('pnpm i --no-frozen-lockfile', { cwd: bellaOSroot, stdio: 'pipe' }).toString().trim();
    ...
    // add to agent
    const agentPackageJsonPath = bellaOSroot + '/agent/package.json'
    ...
    if (!agentPackageJson.dependencies[pluginName]) {
      ...
      try {
        const pluginAddAgentOutput = execSync('pnpm add ' + pluginName + '@workspace:* --filter ./agent', { cwd: bellaOSroot, stdio: 'pipe' }).toString().trim();
        //console.log('pluginAddAgentOutput', pluginAddAgentOutput)
      } catch (e) {
        console.error('error', e)
      }
...
pluginsCmd
  ...
  .action(async (plugin, opts) => {
    ...
    try {
      ...
      const pluginRemoveAgentOutput = execSync('pnpm remove ' + pluginName + ' --filter ./agent', { cwd: bellaOSroot, stdio: 'pipe' }).toString().trim();
    } catch (e) {
