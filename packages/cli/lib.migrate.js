...
function buildPluginFromDir(pluginDir) {
  const corePath = path.join(pluginDir, 'node_modules', '@bellaos', 'core');
...
function migratePlugin(plugin) {
  ...
  if (plugin.references.length > 0) {
    ...
    for (const ref of plugin.references) {
      ...
      try {
        ...
        // Use a regex that will only match the exact string @bellaos/core
        // This prevents double replacements (@bellaos/core-plugin-v1-plugin-v1)
  ...
  if (plugin.hasNodeModulesCore) {
    ...
    console.log(`  Found @bellaos/core in node_modules - will need to reinstall dependencies`);
  }
