...
const OLD_IMPORT = '@bellaos/core';
const NEW_IMPORT = '@bellaos/core-plugin-v1';

// Find all plugin directories
function getPluginDirectories() {
  if (!fs.existsSync(PACKAGES_DIR)) {
    console.error(`Error: Packages directory not found at ${PACKAGES_DIR}.`);
    console.error(`Make sure you're running the script on a valid Bella repository.`);
    process.exit(1);
  }
...
// Replace @bellaos/core with @bellaos/core-plugin-v1 in a file
// nothing calls this
...
function main() {
  ...
  console.log(`\nScanning plugins for @bellaos/core references...`);
  ...
  console.log(`Plugins with @bellaos/core references: ${pluginsWithRefs.length}`);
  console.log('\nPlugins by category:');
  ...
  if (!ANALYZE_ONLY) {
    ...
    for (const plugin of pluginsWithRefs) {
      ...
      // Step 2: For plugins with node_modules/@bellaos/core, reinstall dependencies
      if (results.nodeModules) {
    ...
    console.log(`Plugins with node_modules/@bellaos/core: ${migrationStats.nodeModulesFound}`);
...
Arguments:
  path                    Local directory containing Bella plugins (default: current directory)
...
Examples:
  ...
  node migrate-to-core-plugin-v1.js /path/to/bella            # Migrate plugins in specified directory
  node migrate-to-core-plugin-v1.js --analyze-only            # Analyze only without migrating
  node migrate-to-core-plugin-v1.js https://github.com/user/bella.git  # Clone and migrate GitHub repo
`);
}
