import { DirectClient } from "@bellaos/client-direct";
...
import { bootstrapPlugin } from "@bellaos/plugin-bootstrap";
...
const logFetch = async (url: string, options: any) => {
    bellaLogger.debug(`Fetching ${url}`);
    // Disabled to avoid disclosure of sensitive information such as API keys
    // bellaLogger.debug(JSON.stringify(options, null, 2));
...
export async function loadCharacterFromOnchain(): Promise<Character[]> {
    ...
    try {
        ...
        if (isAllStrings(character.plugins)) {
            bellaLogger.info("Plugins are: ", character.plugins);
...
async function loadCharacterTryPath(characterPath: string): Promise<Character> {
    ...
    if (content === null) {
        ...
        bellaLogger.error("Tried the following paths:");
        pathsToTry.forEach((p) => bellaLogger.error(` - ${p}`));
        throw new Error(
            `Error loading character from ${characterPath}: File not found in any of the expected locations`
        );
    }
...
async function readCharactersFromStorage(
    ...
    try {
        ...
        bellaLogger.error(`Error reading directory: ${err.message}`);
    }
...
export async function loadCharacters(
    ...
    if (hasValidRemoteUrls()) {
        bellaLogger.info("Loading characters from remote URLs");
    ...
    if (loadedCharacters.length === 0) {
        bellaLogger.info("No characters found, using default character");
...
async function handlePluginImporting(plugins: string[]) {
    if (plugins.length > 0) {
        ...
        //bellaLogger.info("Plugins are: ", plugins);
        const importedPlugins = await Promise.all(
            plugins.map(async (plugin) => {
                try {
                    ...
                    const functionName =
                        plugin
                            .replace("@bellaos/plugin-", "")
                            .replace("@bellaos-plugins/plugin-", "")
    ...
    switch (provider) {
        ...
        case ModelProviderName.NEARAI:
            try {
                ...
                bellaLogger.warn(`Error loading NEAR AI config: ${e}`);
            }
            return (
...
export async function initializeClients(
    ...
    // bellaLogger.log("initializeClients", clientTypes, "for", character.name);
        ...
        for (const plugin of character.plugins) {
            if (plugin.clients) {
                for (const client of plugin.clients) {
                    ...
                    bellaLogger.debug(`Initializing client: ${client.name}`);
...
export async function createAgent(
    ...
    bellaLogger.log(`Creating runtime for character ${character.name}`);
...
function initializeCache(
    ...
    switch (cacheStore) {
        ...
        //         bellaLogger.info("Connecting to Redis...");
        ...
        case CacheStore.DATABASE:
            if (db) {
                bellaLogger.info("Using Database Cache...");
        ...
        case CacheStore.FILESYSTEM:
            bellaLogger.info("Using File System Cache...");
            if (!baseDir) {
                throw new Error(
...
async function findDatabaseAdapter(runtime: AgentRuntime) {
    ...
    if (adapters.length === 0) {
        const sqliteAdapterPlugin = await import(
            "@bellaos-plugins/adapter-sqlite"
        );
...
async function startAgent(
    ...
    try {
        ...
        // report to console
        bellaLogger.debug(`Started ${character.name} as ${runtime.agentId}`);
...
const startAgents = async () => {
    ...
    try {
        ...
        bellaLogger.error("Error starting agents:", error);
    }
    ...
    if (serverPort !== Number.parseInt(settings.SERVER_PORT || "3000")) {
        bellaLogger.warn(`Server started on alternate port ${serverPort}`);
    }
...
const setProxy = () => {
    ...
    if (proxy) {
        bellaLogger.info("start agents use proxy : ", proxy);
...
startAgents().catch((error) => {
    bellaLogger.error("Unhandled error in startAgents:", error);
    process.exit(1);
});
